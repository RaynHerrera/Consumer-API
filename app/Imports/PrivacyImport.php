...
class PrivacyImport implements ToModel
{
    /**
    * @param array $row
    *
    ...
    */
    ...
    public function model(array $row)
    {
     ...
     return   $company = Company::create([
            // "id" => 100,
            "company_name" => "shop2shiva",
            "company_logo" => ''
        ]);
    
        //   $company = DB::table('extension_company')->insert([
        //     "id" => 10,
        //     "company_name" => "shop2abhi",
        //     "company_logo" => ''
        //     ]);
        log::alert("rows");die;
        foreach ($rows as $row) {
            ...
            if (!$CompanyUrlName = CompanyUrlName::where('company_url_name', $row['url'])->first()) {
                log::alert('demo-1.0');
                log::alert($row['company_name']);
            
                $company = DB::table('extension_company')->insert([
                "id" => Company::orderBy('id', 'desc')->first()->id +1,
                ...
                "id" => CompanyUrlName::orderBy('id', 'desc')->first()->id +1,
                "company_url_name" => $row['url'],
            ...
            if (!in_array($row['company_name'], $import_company)) {
                ...
                $CompanyUrlNameId = CompanyUrlName::where('company_url_name', $row['url'])->first()->company_id;
                ...
                if ($oldPrivacyStatementData) {
                    ...
                    $oldPrivacyStatementData['created_at'] = Carbon::now();
                    $oldPrivacyStatementData['updated_at'] = Carbon::now();
                ...
                // log::alert($array2);
                //  return   $arrayunique = array_map('strtolower',array_unique(array_merge($array1,$array2), SORT_REGULAR));
                //    $demoarray1 = array_map('trim',array_map('strtolower', array_unique(array_merge($array1,$array2), SORT_REGULAR)));
                //    print_r(str_replace(' ', '_', $demoarray1));
                ...
                if ($oldPrivacyStatementData) {
                    ...
                    $PrivacyStatementData->id =  PrivacyStatement::orderBy('id', 'desc')->first()->id +1;
                }
                ...
                // $PrivacyStatementData->alifornia_consumer_request_information_phone_number= array_search('alifornia_consumer_request_information_phone_number',$excelFeilds) ? true : false;
                // $PrivacyStatementData->california_consumer_request_information_other= array_search('california_consumer_request_information_other',$excelFeilds) ? true : false;
                ...
                // $PrivacyStatementData->for_Non_CA_citizen_action_rights_phone_number= array_search('for_Non_CA_citizen_action_rights_phone_number',$excelFeilds) ? true : false;
                // $PrivacyStatementData->for_Non_CA_citizen_action_rights_link= array_search('for_Non_CA_citizen_action_rights_link',$excelFeilds) ? true : false;
                ...
                $PrivacyStatementData->elroi_internal_date= isset($row['timestamp']) ? $this->transformDate($row['timestamp']) : Carbon::now();
    ...
    public function oldPrivacyStatementData($Company_id)
    {
        $oldPrivacyStatementData = PrivacyStatement::where('company', $Company_id)->first();
        return $oldPrivacyStatementData;
    }
}
